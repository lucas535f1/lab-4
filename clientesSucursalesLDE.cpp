...
TCliente clienteMasRepetido(TClientesSucursalesLDE clientesSucursalesLDE)
{
	...
	while (actual != NULL)
	{
		...
		{
			TCliente visitante = obtenerNesimoClienteTClientesABB(grupoActual, i + 1);

			int cantRepeticiones = 0;
			if (masRepetido == NULL || idTCliente(visitante) != idTCliente(masRepetido))
			{
				cantRepeticiones++;
				...
				while (iterBusqueda != NULL)
				{
					...
					if (existeTClienteTClientesABB(siguienteGrupo, idTCliente(visitante)))
					{
						cantRepeticiones++;
					}
				...
				if (cantRepeticiones > cantMaxRepetido ||
					(cantRepeticiones == cantMaxRepetido && idTCliente(visitante) < idTCliente(masRepetido)))
				{
